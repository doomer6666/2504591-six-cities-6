...
>(Action.FETCH_OFFERS, async (_, { extra }) => {
  ...
  const adapted = adaptOffersToClient(data);
  return adapted;
});
...
export const postOffer = createAsyncThunk<
  ...
  NewOffer,
  { extra: Extra }
>(Action.POST_OFFER, async (newOffer, { extra }) => {
  const { api, history } = extra;
...
export const editOffer = createAsyncThunk<
  ...
  NewOffer & { id: string },
  { extra: Extra }
>(Action.EDIT_OFFER, async (offer, { extra }) => {
  const { api, history } = extra;
  const { data } = await api.patch<OfferDto>(
    `${ApiRoute.Offers}/${offer.id}`,
    adaptEditOfferToServer(offer)
  );
...
>(Action.LOGIN_USER, async ({ email, password }, { extra }) => {
  const { api, history } = extra;
  const { data } = await api.post<UserWithTokenDto>(ApiRoute.Login, {
    email,
    password,
  });
  const { token } = data;
...
export const registerUser = createAsyncThunk<
  ...
  UserRegister,
  { extra: Extra }
>(Action.REGISTER_USER, async (userRegister, { extra }) => {
  const { api, history } = extra;
  await api.post<UserDto>(
    ApiRoute.Register,
    adaptSignupToFormData(userRegister)
  );
...
>(Action.POST_COMMENT, async ({ id, comment, rating }, { extra }) => {
  ...
  const { data } = await api.post<CommentDto>(
    `${ApiRoute.Comments}/${id}/comments`,
    adaptCreateCommentToServer({ comment, rating })
  );
...
export const postFavorite = createAsyncThunk<
  ...
  FavoriteAuth,
  { extra: Extra }
>(Action.POST_FAVORITE, async ({ id, status }, { extra }) => {
  const { api, history } = extra;

  try {
    const { data } =
      status === 1
        ? await api.post<OfferDto>(`${ApiRoute.Offers}/${id}/favorite`)
        : await api.delete<OfferDto>(`${ApiRoute.Offers}/${id}/favorite`);
